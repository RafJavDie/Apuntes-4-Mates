...
Sea $x^*$ mínimo local de $f$, siendo el único punto estacionario en un entorno de $x^*$. Entonces el punto $x^*$ verifica que 
\begin{itemize}
\item $\exists S$ entorno de $x^*$ tal que si $\exists \overline{k}$ con $x^{\overline{k}}\in S$, entonces $x^k\in S\ \forall k>\overline{k}$ y $x^k\to x^*$.
\item Dado $\varepsilon>0$, $S$ se puede elegir para que $||x-x^*||<\varepsilon \ \forall x\in S$.
\end{itemize}
\end{teorema}
\begin{dem}
Como $x^*$ es mínimo local y tiene un entorno donde no hay otro punto estacionario de $f$, podemos tomar $\rho>0$ tal que $\forall x$ $0< ||x-x^*||<\rho$ tenemos $f(x)>f(x^*)$. Definimos la función
$$\phi
(t)=\min_{0\leq t\leq ||x-x^*||\leq\rho} (f(x)-f(x^*))$$
$\phi$ \textbf{es no decreciente}, pues si $t$ disminuye, la corona circular se hace más gruesa hacia dentro, por lo que hay más puntos donde encontrar el mínimo, y si $t$ aumenta, la corona se hace más fina por lo que hay menos puntos donde encontrar el mínimo.

Por la continuidad de de la norma y $\nabla f$, sabemos que $\forall \varepsilon>0\ \exists r>0\mid ||x-x^*||<r$ tal que $|g(x)-g(x^*)|<\varepsilon$, donde $g(x)=||x-x^*||+sc||\nabla f(x)||$. Se tiene que $g(x^*)=0$. Luego
...
\end{itemize}
Por lo que $x^{k+1}\in S$. Además $S$ está contenido en un compacto (una bola cerrada de radio $\varepsilon$), por lo que toda sucesión contiene una subsucesión convergente, esto es, $\{x^l\}_{l>k}$ contiene una subsucesión convergente. Y como $x^*$ es el único punto estacionario, la sucesión debe converger hacia dicho punto.\newline $\QED$
\end{dem}
...
\end{teorema}
\begin{dem}
Por la definición de $f$, $\nabla f(x)=Qx$. En la iteración $k$, $f(x^k)=\frac{1}{2}x^{k'}Qx^k$, luego $\nabla f(x^k)=Qx^k:=g^k$. Podemos escribir $f(x^k)=\frac{1}{2}x^{k'}QQ^{-1}Qx^k=\frac{1}{2}g^{k'}Q^{-1}g^k$. Notemos que si $g^k = 0$ si y solo si $x^k=0$, y siendo $Q$ definida positiva, esto es si y solo si $x^0=0$. Podemos por tanto obviar este caso.
...
En definitiva, $\forall\varepsilon>0, ||x^{k+1}-x^*||\leq\varepsilon||x^k-x^*||$, lo que significa $$\lim_{k\to\infty}\frac{||x^{k+1}-x^*||}{||x^k-x^*||}=0.$$
...
\end{align*}
Supongamos que $\frac{\partial f(x^k)}{\partial x_j}\leq\frac{\partial f(x^k)}{\partial x_i}\ \forall i$, entonces $x^*_j=r, x^*_i=0\ \forall i\neq j$.
