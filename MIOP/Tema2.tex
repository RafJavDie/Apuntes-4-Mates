...
\end{teorema}
\begin{dem}
Como $x^*$ es mínimo local y el único punto estacionario en un entorno suyo, podemos tomar $\rho>0$ tal que $\forall \mid 0<||x-x^*||\leq\rho$, entonces $f(x)>f(x^*)$. Definimos la función $$\phi
...
\end{align*}
Supongamos que $\frac{\partial f(x^k)}{\partial x_j}\leq\frac{\partial f(x^k)}{\partial x_i}$ $\forall i$, entonces $x^* = r\cdot e_j$ es solución del problema. 
\end{ej}
