...
\item Si $t_i^k > t_i$ algún $i,k$; tomar $\delta(i)=k$ y $t_i = t^k_i$.
...
Vectorialmente aplicando el lema anterior ($w=g$) tenemos que $g\geq R^\delta+Q^\delta g$ $\forall g$, $R^\delta+Q^\delta g\geq v^\delta$, por lo que $\geq v^\delta$ $\forall \delta \in \Delta$. Pero como $g$ alcanza el valor de una política $(\pi)$ entonces $g$ es único ($g=v^\pi$).
\end{dem}
\begin{teorema}
...
$$
f_i \geq R_i^k + \sum_{j=1}^N \alpha p_{ij}^k f_k \quad \forall k\in D(i)$$
