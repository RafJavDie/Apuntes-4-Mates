...
\end{teorema}
\begin{dem}(Hacer dibujos para cuando se lo mande a mcgarcia)
...
\end{teorema}
\begin{dem} (Hacer dibujos para mcgarcia)

$\boxed{\Rightarrow}$ Supongamos que $p_i$ no está en la envolvente convexa. Vamos a probar que $Vor(p_i)$ es acotada. Sean $p_1,p_2,p_3\in S$ tales que $p_i$ está en el interior del triángulo delimitado por los 3 vértices anteriores. Llamamos $c_{kj}$ a la circunferencia que pasa por $p_i,p_j,p_k,\ j,k=1,2,3,\ j\neq k$. Tomamos una recta $C$ que pase por $p_i$ y un punto $x$ exterior a las circunferencias. Entonces esa recta corta a alguna circunferencia en (al menos) un punto. $d(x,p_i)<d(x_i,p_2)$

$\boxed{\Leftarrow}$ Supongamos que $Vor(p_i)$ es acotado. Entonces por el dibujo (dibujar p, un polígono alrededor y puntos fuera), $p_i$ no está sobre la envolvente convexa. $\QED$
\end{dem}
