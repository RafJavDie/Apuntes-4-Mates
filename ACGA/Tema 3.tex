...
\begin{ejercicio}
 Sean $Y=\AAA^2_\CC$ y $Z=Y-\{(0,0)\}$. Por el ejercicio \ref{a2menos0} $\OO_{Z}(Y)=\OO_{Z}(Z)=\CC[x,y]$. Probar que no existe ningún morfismo $\phi:Y\to Z$ tal que $\phi^\star:\CC[x,y]\to\CC[x,y]$ sea la identidad.
\end{ejercicio}


\begin{eje}\label{ejemorf}
\emph{
 \begin{enumerate}
  \item La aplicación $\phi:x\mapsto (x,1/x)$ es un morfismo entre $\AAA^1_k-\{0\}$ y $\VV(\{xy-1\})\subseteq\AAA^2_\CC$.
...
Recíprocamente, supongamos que $\phi^\star$ es inyectivo. Sea $f\in\calA(Z)$ una función regular que se anule en todo punto de la imagen de $\phi$, es decir, tal que $f\circ\phi=\phi^\star f=0$. Por ser $\phi^\star$ inyectiva, $f$ debe ser nula en todo $Z$. Es decir, $\II(\phi(Y))\subseteq\II(Z)$. Aplicando $\VV$ y usando el ejercicio \ref{clausura} concluimos que $Z\subseteq\overline Z\subseteq\overline{\phi(Y)}$, es decir, $\phi(Y)$ es denso en $Z$. 
\end{proof}
...
\begin{eje}
 \emph{
 \begin{enumerate}
\item Si $\phi:\AAA^2_\CC-\{(0,0)\}\to\AAA^2_\CC$ es el morfismo dado por la inclusión, $\phi^\star:\CC[x,y]\to\CC[x,y]$ es un isomorfismo de $\CC$-álgebras (la identidad) por el ejercicio \ref{a2menos0}.
  \item El morfismo $\phi$ del ejemplo \ref{ejemorf} (1) es un isomorfismo: su inverso viene dado por $(x,y)\mapsto x$. 
\item Si $Z\subseteq\Ank$ es una hipersuperficie, la variedad cuasi-afín $\Ank-Z$ es isomorfa a una variedad afín: en efecto, si $Z$ viene definida por la ecuación $f=0$, el morfismo $(x_1,\ldots,x_n)\mapsto(x_1,\ldots,x_n,1/f(x_1,\ldots,x_n))$ es un isomorfismo entre $\Ank-Z$ y la hipersuperficie afín de $\AAA^{n+1}_k$ definida por la ecuación $x_{n+1}f(x_1,\ldots,x_n)-1=0$.
 \end{enumerate}
}
\end{eje}
...
\begin{ejercicio}
 Sea $\phi:\AAA^1_\CC\to\VV(\{y^2-x^3\})\subseteq\AAA^2_\CC$ el morfismo dado por $t\mapsto (t^2,t^3)$. Probar que $\phi$ es biyectivo, incluso un homeomorfismo, pero no es un isomorfismo.
\end{ejercicio}
...
\end{defn}
 ...
 \begin{eje}\emph{
  Sea $Z\subseteq\AAA^3_\CC$ la cuádrica definida por la ecuación $z^2=xy$. Las aplicaciones racionales $\AAA^2_\CC\to Z$ dada por $(a,b)\mapsto(\frac{b^2}{b^2-a},\frac{a^2}{b^2-a},\frac{ab}{b^2-a})$ y $Z\to\AAA^2_\CC$ dada por $(x,y,z)\mapsto (\frac{y}{x-1},\frac{z}{x-1})$ son inversas la una de la otra, y por tanto son birracionales. En particular, $Z$ es racional.}
 \end{eje}
...
\begin{prop}
 Una aplicación racional $\psi:X\dashrightarrow Y$ viene dada por $m+1$ polinomios $f_0,f_1,\ldots,f_m\in k[x_0,x_1,\ldots,x_n]$ homogéneos del mismo grado tales que $\langle f_0,f_1,\ldots,f_m\rangle\not\subseteq\II(X)$, mediante la fórmula $\psi(x_0:\ldots:x_n)=(f_0(x_0,\ldots,x_n):\ldots:f_m(x_0,\ldots,x_n))$. La aplicación $\psi$ está definida en el conjunto $X\backslash\VV(\{f_0,\ldots,f_m\})$ de puntos $x\in X$ en los que algún $f_j$ no se anula. Otro conjunto de polinomios $g_0,g_1,\ldots,g_m$ homogéneos del mismo grado define la misma función racional si y sólo si $f_ig_j-f_jg_i=0$ en $X$ para todos $0\leq i<j\leq m$. 
\end{prop}
