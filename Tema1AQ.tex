...
\end{equation*}
\item[c)] Solo resta ver que $\tilde{f}^{-1}$ es continua, para lo cual veremos que $\tilde{f}$ es cerrada. Por las hipótesis sobre $X$ e $Y$ y la Proposición \ref{125} deducimos que $f$ es cerrada. Sea $K$ un cerrado de $X/\mathcal{R}_f\Rightarrow\pi^{-1}(F)$ es cerrado de $X$. Por ser $f$ cerrada, $f(\pi^{-1}(K))$ es cerrado, pero precisamente $\tilde{f}(F)=f(\pi^{-1}(F))$, luego $\tilde{f}$ es cerrada y, por tanto, homeomorfismo. 
{$\QED$}
\end{enumerate}
