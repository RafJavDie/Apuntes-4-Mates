...
$$
\sum_{n=1}^\infty  \frac{μ(n)}{\senh nx} = \sum_{n=1}^\infty \mu(n) \sum_{m=1}^\infty f(nmx) 
$$
\end{solucion}
...
\begin{align*}
	...
	\sum_{n=1}^{∞} μ(n) u(n) \left(\sum_{m=1}^\infty u(m) f\left(\frac{x}{mn}\right)\right) = \sum_{mn=1}^{∞} μ(n) u(mn) f\left(\frac{x}{mn}\right)\\
