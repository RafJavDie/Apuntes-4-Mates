...
& = 2 \sum_{n=1}^{∞} μ(n) \sum_{k=1}^{∞} e^{-(2k-1)nx}= 2 \sum_{n=1}^{∞} \sum_{k=1}^{∞} μ(n) e^{-(2k-1)nx}
\end{align*}
>>>>>>> 4fcc691... Algunas consideraciones en 10 y 12, pero sin acabar ambos.
...
=======
 ...
 \[ \sum_{d|p} μ(p/d) σ(d) = 1+(1+p) = p \]
 Lo que demuestra la fórmula.
>>>>>>> 90af629... Creo que he resuelto bien todos los conflictos pero Diego, comprueba si echas algo de menos
\end{solucion}
