...
--data Arbol = Nodo Int [Arbol]
--instance Show Arbol where
--    show a = init (aux 0 a)
--       where aux k (Nodo n as) = replicate k ' ' ++ show n ++ "\n" ++ concatMap (aux (k+1)) as

--eja = Nodo 1 [Nodo 2 [Nodo 3 [], Nodo 5 []], Nodo 4 [Nodo 6 []], Nodo 7 []]
...
--caminos :: Arbol -> [[Int]]
--caminos (Nodo n []) = [[n]]
--caminos (Nodo n as) = map (n:) (concatMap caminos as) 
...
--arbol1 :: [[Int]] -> [Arbol]
--arbol1 [] = []
--arbol1 as = let cas = filter (not.null) as in
--    [Nodo n (arbol1 [tail a | a <- as, head a == n]) | n <- nub (map head cas)]

--nivel :: Arbol -> Arbol
--nivel = arbol . concatMap fp . caminos

--niveles :: Int -> Arbol -> Arbol
--niveles n = (!!n) . iterate nivel

--gilarbol :: Arbol
--gilarbol = Nodo 2 [Nodo 3 []]
