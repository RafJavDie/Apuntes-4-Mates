...
\begin{quotation}
\noindent En este trabajo presentamos la conjetura de Gilbreath sobre las diferencias en valor absoluto de primos consecutivo. Ofrecemos un algoritmo eficiente para comprobar la conjetura. Introducimos una generalización de la conjetura reemplazando la sucesión de primos por una sucesión cualquiera. Ofrecemos un estudio teórico y computacional de la generalización de la conjetura en estas sucesiones.
\end{quotation}
...
\end{nota}
\begin{nota}
Esta propiedad no parece ser algo intrínseco a los primos gemelos. En los casos estudiados, definiendo análogamente la sucesión $Q_n$ para una sucesión de Gilbreath cualquiera, parecen tener un comportamiento similar cuando $a_n = a_{n-1}+2$ suele decrecer igual que ocurre con los primos gemelos, en comparación a cuando no se escoge $a_{n-1}+2$.
\end{nota}
\subsection{Estudio probabilístico}
En esta sección vamos a interesarnos por secuencias $a_n$ de longitud $N$ grande cuya distribución de saltos $a_{n+1}-a_n$ se asemeja a la distribución de saltos de $p_{n+1}-p_n$.
...
Ejecutando el anterior programa obtenemos que la probabilidad de obtener una sucesón de Gilbreath que sigue la misma distribución de saltos que la sucesión de primos es $0.23$\%. Si en lugar de partir de $\{2,3\}$ partimos de $\{2,3,5\}$ la probabilidad aumenta a $1.39$\%. Si tomamos los primeros $4$ primos, la probabilidad se vuelve $2.07$\%. Resulta destacable que los saltos de los primeros números primos son a la vez improbables en su distribución y necesarios para que la sucesión sea de Gilbreath. 
\newpage
