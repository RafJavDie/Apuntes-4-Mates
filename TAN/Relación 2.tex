...
\end{ejercicio}
\begin{sol}
Dividimos los números de $1$ a $x$ en grupos de $k$ elementos. Como puede que $k$ no divida a $x$, el último grupo puede tener menos de $k$ elementos. Tendremos entonces $\lfloor x/k \rfloor$ grupos de $k$ elementos. Obsérvese que entre $nk+1$ y $(n+1)k$ hay $φ(k)$ primos siempre y cuando $n ≥ 1$. Acotamos la cantidad de primos entre $1$ y $k$ y en el último grupo por $2k$ (como si todos fueran primos). Entonces:
\[ π(x) ≤ φ(k) \left\lfloor \frac{x}{k} \right\rfloor+2k ≤ φ(k) \frac{x}{k} + 2k \]
...
\[ \sum_{p≤x} \sqrt{p}\log p = \sum_{n≤x} \sqrt{n}a_n = θ(x)\sqrt{x} - \int_1^x θ(t)\frac{1}{2\sqrt{t}}dt = \int_2^x θ(t)\frac{1}{2\sqrt{t}}dt \]
(el cambio del rango de la integral a $2$ será evidente más adelante). Usando el ejercicio anterior, $θ(t)=t+U(t)$, con $|U(t)| ≤ C (x^{1/2}\log^2x)$ para algún $C$:
\[ \sum_{p≤x} \sqrt{p}\log p = θ(x) \sqrt{x} - \frac{1}{2} \int_2^x \sqrt{t} dt - \frac{1}{2} \int_2^x \frac{U(t)}{\sqrt{t}} \]
Como $int_1^x \frac{U(t)}{\sqrt{t}}≤C\int_2^x \log^2 t dt ≤ C x \log^2 x$ para $x ≥ 2$:
\[ \sum_{p≤x} \sqrt{p}\log p = (x+\mathcal{O}(\sqrt{x}\log^2x))\sqrt{x}-\frac{1}{3}x^{3/2}+\frac{1}{3}2^{3/2}+\mathcal{O}(x\log^2 x) = \frac{2}{3} x^{3/2} + \mathcal{O}(x\log^2 x) \]
\end{sol}
