...
\[ = \left|\frac{s_n}{n^s}-\frac{s_m}{(m+1)^s}+\sum_{k=m+1}^{n-1}s_k\left(\frac{1}{k^s}-\frac{1}{(k+1)^s}\right)\right| ≤ \frac{\varepsilon}{n^σ} + \frac{\varepsilon}{(m+1)^σ} + \sum_{k=m+1}^{n-1} \varepsilon \left|\frac{1}{k^s}-\frac{1}{(k+1)^s}\right|\]
...
\begin{align*}
 \left|-\frac{a_N}{N^σ}\right| &= \left|\sum_{n=N+1}^{∞} \frac{a_n}{n^s}\right| ≤ \sum_{n=N+1}^{∞} \frac{|a_n|}{n^σ}≤ c \sum_{n=N+1}^{∞} \frac{1}{n^{σ-α}} \\
...
$$
\prod_{p\leq x}\left(1-\frac{f(p)}{p^s}\right)^{-1} =  \sum_{P^+(n)≤x} \frac{f(n)}{n^s} 
$$
Estudiamos ahora 
\begin{align*}
...
\end{prop}
\begin{dem}
El orden mínimo es claro, pues $d(n)\geq 2$. Vamos a demostrar el orden máximo. Vamos a ver que
