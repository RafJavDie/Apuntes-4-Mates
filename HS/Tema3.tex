...
\[ c = \sum_{\dim(µ \in K_2)=2} µ \]
en $K_2$ se comprueba que $\partial c=\sigma+2\tau$, por lo que $[\sigma]=2[-\tau]$ en $H_1(K_2)$. Así que $i_*([\sigma])=2[-\tau]$ con lo que tenemos que  $i_*$ es inyectiva. Por exactitud esto implica que $H_2(K)\cong\Ima\Delta_{*2}=\ker i_*=0$. Ahora, usando el primer teorema de isomorfía y la sobreyectividad de $j_{1*}$ tenemos que
\[
H_1(K)\cong \F^2\gene{\tau}/2\F\gene{\tau}=0
\]
por ser $\F$ cuerpo. En conclusión, $\widetilde{H}_q(K)=0$ para todo $q$.
\end{ej}
