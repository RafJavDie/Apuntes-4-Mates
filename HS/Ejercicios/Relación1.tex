...
Supongamos que $|K|$ es conexo. Dado $x_0\in|K|$, sea $U_{x_0}=\{y\in|K|\mid\exists$ camino de $x_0$ a $y\}$. Veamos que
\begin{enumerate}
\item $U_{x_0}\neq\emptyset$ para todo $x_0\in|K|$, ya que existe el camino constante en $x_0$, así que $x_0\in U_{x_0}$.
\item $U_{x_0}$ es abierto, es decir, vamos a ver que dado $y\in U_{x_0}$ existe un abierto entre $y$ y $U_{x_0}$. Como $y\in U_{x_0}$, existe un camino $\alpha:[0,1]\to |K|$ tal que $\alpha(0)=x_0$ y $\alpha(1)=y$. Consideremos la estrella abierta de $y$ en $|K|$, dado que un símplice es conexo, cualquier punto de la estrella abierta de $y$ está conextado mediante un segmento con $y$, luego añadiendo $\alpha$, cualquier punto de la estrella abierta de $y$ está conectado con $x_0$, luego $y\in\mathring{st}(y,|K|)\subseteq U_{x_0}$. 
\item $U_{x_0}$ es cerrado, si y solo si $|K|\setminus U_{x_0}$ si y solo si $|K|\mid U_{x_0}$ es entorno de todos sus puntos. Dado $y\in |K|\mid U_{x_0}$, basta considerar $\mathring{st}(y,|K|)\subseteq|K|\setminus U_{x_0}$, ya que de lo contrario, por ser la estrella abierta conexa por caminos, existiría un camino entre $x_0$ e $y$.

Veamos que entonces el 1-esqueleto es conexo. Dados $x,y\in |K^1|$ sabemos que existe un camino $\alpha:[0,1]\to |K|$ de $x$ a $y$. Por el teorema de aproximación simplicial, existe una subdivisión $sd^n[0,1]$ y $\varphi:sd^n[0,1]\to |K|$ que es aproximación de $\alpha$. Como $sd^n[0,1]$ está formado solo por vértices y aristas, la imagen también debe estar formada por vértices y aristas, luego su imagen es un camino en $K^1$. Es posible que $\varphi(0)\neq x$ y $\varphi(1)\neq y$ si están en el interior de una arista, pero en cualquier caso, se podrá hacer un camino entre $\varphi(0)$ y $x$, y entre $\varphi(1)$ e $y$ dentro de la arista en la que se encuentran. 
\end{enumerate}
