...
Nuestros objetos serán $\R$-espacio vectoriales de dimensión finita (ó $R$ módulos con $R$ dominio de ideales principales).

\begin{defi}
Dado un diagrama de $\R$-e.v. y homomorfismos:
\[ M_{i-1} \xrightarrow{f_{i-1}} M_i \xrightarrow{f_i} M_{i+1} \]
se dice que la sucesión es exacta en $M_i$ si $\ker f_i = \Ima f_{i-1}$. Si
...
es exacto para todo $i \in \Z$, entonces diremos que es una sucesión exacta larga.
\end{defi}
...
\item $0 \rightarrow M_1 \xrightarrow{f_1} M_2 \xrightarrow{f_2} M_3 \rightarrow 0$ es exacto sii $f_1$ es inyectiva, $f_2$ es sobreyectiva y $f_2$ induce un isomorfismo entre $\coker f_1$ y $M_3$. Este tipo de sucesiones es conocido como \textbf{sucesión exacta corta}.
\end{enumerate}
\end{lemma}
\begin{dem}
\begin{enumerate}[a)]
...
\item $0 \rightarrow M_1 \xrightarrow{f_1} M_2 \xrightarrow{f_2} M_3 \rightarrow 0$ es exacto sii $f_1$ es inyectiva (apartado (a)), $f_2$ es sobreyectiva (apartado (b)) y $\ker f_2 = \Ima f_1$, es decir $\coker f_1 = M_2/\Ima f_1 = M_2/\ker f_2 \cong \Ima f_2 = M_3$ (por el primer teorema de isomorfismo).
\end{enumerate}
\end{dem}
...
Véase que hemos probado con toda sucesión exacta corta de espacios vectoriales \textbf{escinde}. Usaremos este lema (demuéstrese como ejercicio):
\begin{lemma}[Splitting Lemma]
Se dice que $0 \rightarrow V_1 \xrightarrow{f} V_2 \xrightarrow{g} V_3 \rightarrow 0$ \textbf{escinde} si se cumple uno de las siguientes condiciones equivalentes:
\begin{enumerate}
...
Sea $m_3 \in M_3$ tal que $ψ_3(m_3) = 0$, luego $0=φ_3ψ_3(m)=φ_4θ_3(m_3)$.
Como $φ_4$ es isomorfismo, $θ_3(m_3)=0$.
...
donde $C_i$ son $\R$-e.v. llamados \textbf{$n$-cadenas}, $δ_i$ son homomorfismos de $\R$-e.v. llamados \textbf{operadores borde}, tal que $δ_n \circ δ_{n+1} = 0$ para todo $n \in \Z$. En particular, decimos que $\mathcal{C}_* = \{(C_n,δ_n)\}_{n \in \Z}$.

Para todo $n \in \Z_+$, denotamos $Z_n := \ker δ_n \subseteq C_n$. A los elementos de $Z_n$ los llamamos \textbf{$n$-ciclos}.
Definimos $B_n := \Ima δ_{n+1} \subseteq C_n$, cuyos elementos se llaman \textbf{$n$-bordes}.
Como $δ_n \circ δ_{n+1} = 0$, tenemos que $B_n \subseteq Z_n$.
De esta manera, definimos $H_n(\mathcal{C}) = Z_n / B_n$ como el \textbf{$n$-ésimo $\R$-e.v. de homología} del complejo de cadenas $\mathcal{C}$.
\end{defi}
