...
\begin{defi}
Un complejo de $\mathcal{C}=\{(C_n,\partial_n)\}_{n\in\Z}$ se dice \emph{positivo} si $C_n=0$ para todo $n>0$. En este cas, diremos que una aplicación lineal sobreyectiva $\varepsilon: C_0\to\mathbb{F}$ cumpliendo $\varepsilon\partial_1=0$ se dirá que es un \emph{aumento} de $\mathcal{C}$.
\end{defi}
...
\]
podemos aumentarlo a un complejo $\tilde{\mathbb{C}}$
\[
...
\]
Donde $\tilde{C}_n=C_n$ para $n\geq 0$ y $\tilde{C}_{-1}=\mathbb{F}$. A los espacios vectoriales $H_n(\tilde{\mathbb{C}})$ también los denotaremos por $\tilde{H}_n( \mathcal{C})$ y los llamaremo espacios vectoriales de homología \emph{reducida} de $\mathcal{C}$. 
...
\begin{defi}
Un morfismo $f \colon \CC^1 \to \CC^2$ de complejos de cadenas se dice \textbf{equivalencia de homotopía} si $\exists g \colon \CC^2 \to \CC^2$ morfismo de complejos de cadenas tal que $f \circ g \simeq id_{\CC^2}$ y $g \circ f \simeq id_{\CC^1}$.

Diremos que $\CC^1$ y $\CC^2$ son homotópicamente equivalentes si existe una equivalencia de homotopía entre ellos.
\end{defi}

Es decir, a los isomorfismos de complejos de cadenas se llaman ``equivalencias de homotopía''.
...
\item Como consecuencia, si $f \colon \CC^1 \to \CC^2$ es equivalencia de homotopía, entonces $f_* \colon H_n(\CC^1) \to H_n(\CC^2)$es un isomorfismos de espacios vectoriales para todo $n \in \Z$.
\end{enumerate}
\end{prop}
\begin{proof}
\begin{enumerate}
\item Sea $z \in Z_n(\CC^1)$:
\[ f_*([z]) - g_*([z]) = [f_n(z)]-[g_n(z)] = [\partial_{n+1}^2(h_n(z)) + h_{n-1}(\partial_n^1(z))] \]
...
\begin{defi}
Un complejo de cadenas $\CC = \{(C_n, \partial_n)\}_{n \in \Z}$ se dice \textbf{contráctil} si $id_{\CC} \simeq 0$.
\end{defi}
...
\[ (id_{C_n}-0)(z) = \partial_{n+1}(h_n(z)) + h_{n-1}(\partial_n(z)) \]
\[ z = \partial_{n+1}(h_n(z)) \in B_n(\CC) \]
Luego $[z] = [\partial_{n+1}(h_n(z))] = [0] \in H_n(\CC)$.

\item Sabemos $Z_n(\CC) = B_n(\CC)$ para todo $n \in \Z$.
Consideramos las sucesiones exactas cortas:
\[
\begin{tikzcd}
0 \arrow[r] & Z_n(\CC) \arrow[r,"i_n"] & C_n \arrow[r,"\partial_n"] & B_{n-1}(\CC) \to 0\\
0 \arrow[r] & Z_{n+1}(\CC) \arrow[r,"i_n"] & C_{n+1} \arrow[r,"\partial_n"] & B_n(\CC) \to 0
\end{tikzcd}
\]
...
\begin{prop}
Sean $\CC = \{(C_n, \partial_n)\}_{n \in \Z}$ y $\CC' = \{(C_n',\partial_n')\}_{n \in \Z}$ complejos de cadenas junto con un morfismo $f \colon \CC \to \CC'$ de complejos de cadenas. En estas condicioens definimos un nuevo complejo de cadenas $\CC_f = \{D_n,\overline{\partial}_n)\}_{n \in \Z}$ dado por:
