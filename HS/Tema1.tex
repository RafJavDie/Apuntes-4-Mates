...
\begin{defi}
Dado un complejo simplicial $K$ se define la primera subdivisión baricéntrica de $K$ como el complejo simplicial $sd K$ formado por todos los síplices del tipo $(b(σ_0),b(σ_1),\dots,b(σ_k))$ donde $σ_0 < σ_1 <\dots <σ_k$ son símplices de $K$.
\end{defi}
\begin{figure}[H]
\centering
\begin{tikzpicture}
...
\draw (2,2) node[anchor=north]{$b(v_0,v_1,v_2)$};
\end{tikzpicture}
\end{figure}
...
Sea $x \in |K|$, sea $σ$ el soporte de $x$ en $K$ y calculamos $b(σ)$. (Seguimos en la siguiente clase)
\end{enumerate}
\end{dem}
