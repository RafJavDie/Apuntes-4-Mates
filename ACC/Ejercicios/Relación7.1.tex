...
$$
\sum_{k=0}^{j-1} (-1)^{j}\sigma_{k}s_{j-k} + (-1)^j j \sigma_j = 0, \quad j\geq 1 
$$
...
\item[]
\item
\item Para $j=1$ es claro, pues tenemos simplemente $\sigma_0 s_1 - s_1 = 0$. Supongamos que es cierto para $j=s-1$ y veamos que es cierto para $j=s$. De la hipótesis de inducción podemos deducir que
\begin{align*}
\sum_{k=0}^{j-1} (-1)^{j}\sigma_{k}s_{j-k} + (-1)^j j \sigma_j  
&= -(-1)^{j-1}(j-1)\sigma_{j-1} + (-1)^{j-1}\sigma_{j-1}s_1 + (-1)^j j \sigma_j\\
&=(-1)^{j-1}\sigma_{j-1}[-(j-1)+s_1] + (-1)^j j \sigma_j
\end{align*}
