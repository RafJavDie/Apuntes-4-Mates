...
$$
Luego $LT(S)$ el múltiplo de $LT(g)$. El otro caso es análogo. Supongamos ahora que ambos tienen dos términos al menos. Como $LM(f)$ y $LM(g)$ son coprimos, podemos suponer sin pérdida de generalidad que $multideg(f)>multideg(g)$. Sean 
$$
f^* = LT(f-LT(f)) \qquad g^* = LT(g-LT(g))
$$
ESTE EJERCICIO NO ESTÁ TERMINADO
\end{enumerate}
\end{solucion}
