...
%\item Por el apartado anterior y por el Nullstellensatz $\I(V)=\sqrt{\gene{x^3-y^2, x^4-z^2, y^4-z^3}}$. Supongamos que $f\in \R[x,y,z]$ y $f(t^2,t^3,t^4)=0$ para todo $t\in\R$. Tenemos que $f(x,y,z)=\sum_{i,j,k}a_{i,j,k}x^iy^jz^k$. Entonces 
%\[
%f(t^2, t^3,t^4)=\sum_{i,j,k}a_{i,j,k}t^{2i}t^{3j}5t^{4k}=\sum_{i,j,k}a_{i,j,k}t^{2i+3j+4k}=0
%\]
%Entonces para cada entero $l\geq 0$ tenemos
%\[
%\underset{2i+3j+4k=l}{\sum_{i,j,k\geq 0}}a_{i,j,k}=0
%\]
%Esto fijo que no lleva a ninguna parte.
\end{enumerate}
\end{solucion}
