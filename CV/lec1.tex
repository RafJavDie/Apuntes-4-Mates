...
\begin{lemma} Sea $w$ una aplicación $r$-lineal tal que $w(v_1,\dots, v_r)=0$ si $v_i=v_{i+1}$ para algún $i$. Entonces $w$ es alternada.
\end{lemma}
\begin{proof}
Repidiendo el lema anterior, 
...
\item $w_1\land (w'+w_3)=(w_1\land w')+(w_1\land w_3)$.
...
\end{lemma}
\begin{proof}
Sea $\tau\in S(p,q)$ tal que $\tau(1)=p+1,\dots, \tau(q)=p+q$, $\tau(q+1)=1,\dots,\tau(q+p)=p$. Se induce la biyección
\begin{gather*}
...
$$w_1(v_{\sigma\tau(q+1)},\dots,v_{\sigma\tau(q+p)})=w_2(v_{\sigma(1)},\dots, v_{\sigma(p)})$$
\begin{gather*}
...
\begin{defi}
Una \textbf{$k$-álgebra} $A$ es un $k$-espacio vectorial con una aplicación lineal 
$$\mu:A\to A$$
que sea asociativa, es decir, $\mu(a,\mu(b,c))=\mu(\mu(a,b),c)\ \forall a,b,c\in A$. Si además $1\in A$, entonces se dirá que $A$ es un \textbf{álgebra unitaria}.
...
Un álgebra graduada $A_*$ se dice \textbf{conexa} (o coercitiva) si existe $\varepsilon:k\to A_0$ que es isomorfismo dado por $\varepsilon(r)=r\cdot 1$ ($1\in A_0$).
...
\end{gather*}
Sea $S(\hat{p}, q,r)\subseteq S(p,q,r)$ las permutciones $\sigma\in S(p,q,r)$ tal que $\sigma(1)=1,\dots,\sigma(p)=p$ y $S(p,q,\hat{r})$ aquellas que $\sigma(p+q+1)=p+q+1,\dots, \sigma(p+q+r)=p+q+r$. Tenemos las siguientes biyecciones
\begin{gather*}
...
\sum_{\hat{\sigma}\in S(p,q,r)} sgn(\hat{\sigma})w_1()w_2()w_3()=\\
...
\end{gather*}
donde el gorro significa que se elimina. Esto es justamente el desarrollo del determinante que buscamos.
\end{proof}
