...
\begin{itemize}
\item Por el Ejerci%cio 2.2 la inclusión es diferenciable. 
...
\end{ejercicio}
\begin{solucion}
Vamos a aplicar el lema de factorización para ver es suficiente que $f(M)\subseteq N$. Supongamos que $M$ es subvariedad con una aplicación $\iota$ y $N$ lo es con $g$. Sea $(\iota(M),i)$ tal que el representante de Imponiendo la condición anterior obtenemos el siguiente diagrama, donde $f|_M$ es la única aplicación que lo cierra
\[
\begin{tikzcd}
...
\end{tikzcd}
\]
Como $f$ es continua (por ser diferenciable), $f|_M$ también, luego aplicando el 1 del Lema de Factorización obtenemos el resultado.
\end{solucion}
...
\end{ejercicio}
\begin{solucion}
Para $z<2$ $M=\emptyset$, por lo que no puede ser variedad. Para $z=2$, $M$ es el conjunto unitario $\{(0,0,2)\}$, que no puede ser de dimensión 1. Para $z>2$, $M$ es una circunferencia de radio $\sqrt{a-2}$. Por el Problema 2.1, la circunferencia tiene una estructura de variedad diferenciable de dimensión 1 análoga a la de $S^1$ con la topología euclídea, luego $(M,i)$ con la inclusión es subvariedad regular por la Proposición 4.2.2.
\end{solucion}
