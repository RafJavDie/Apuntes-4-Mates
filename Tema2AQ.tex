...
\begin{teorema}
Una superficie compacta y conexa distinta de $S^2$ y $\Pro_2\R$ es homeomorfa a un espacio cociente de alguno de los siguientes tipos:
\begin{itemize}
\item \textbf{Tipo I ($M_n$)}
El resultado de identificar los lados de un polígono (llamado polígono fundamental) de $4n$ lados de acuerdo al código $a^{}_1 b^{}_1 a^{-1}_1 b^{-1}_1\dots\ a^{}_n b^{}_n a_n^{-1} b_n^{-1}$. 
...
El resultado de identificar los lados de un polígono de $2n$ lados ($n\geq 2$) por el código $a_1 a_1 \dots\ a_n a_n$.
\begin{center}
\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1.0cm,y=1.0cm]
...
\begin{defi}
La \textbf{suma conexa} de dos superficies $S$ y $S'$ es una nueva superficie $S\# S'$ que resulta al eliminar un disco abierto en cada una de ellas y pegarlas por las circunferencias de los bordes.
\end{defi}
...
$M\# S^2$ es homeomorfa a $M$ ya que $S^2$ con un agujero es homeomorfa a un disco.
\begin{figure}[h!]
...
\draw [->] (0.,0.) -- (0.,1.0498216990612714);
\draw [->] (3.,0.) -- (3.,1.0978053062867201);
\draw [->] (3.,0.) -- (1.4918246161084876,0.);
\draw [->] (4.5133333333333345,0.) -- (4.519545661146498,0.9380614997877437);
...
%\end{figure}
\begin{figure}[h!]
	\includegraphics[scale=0.7]{kleinbottle}
\end{figure}

$C$ es la intersección aparente necesaria para tener este modelo en $\R^3$.
