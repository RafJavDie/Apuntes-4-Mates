...
\end{exercise}
\begin{sol*}
Por el teorema de clasificación de superficies sabemos que solo existen dos tipos de superficies compactas sin borde. Dentro de las del tipo I (orientables) tratamos la esfera aparte, pues ya sabemos que $\pi_1(S^1)\cong\{1\}$. Calculemos pues el grupo fundamental de una superficie orientable arbitraria.\
...
\[
G^{ab}=\langle a\mid a^2\rangle\cong\Z_2.
\]
Esto quiere decir que la primera superficie es un plano proyectivo. 
\[
H^{ab}=\langle a,b,c \mid (abc)^2,aba^{-1}b^{-1},aca^{-1}c^{-1},bcb^{-1}c^{-1}\rangle,
\]
Haciendo el cambio de generador $\gamma=abc$, obtenemos el grupo 
\[
H^{ab}=\langle a,b,\gamma|\gamma^2,aba^{-1}b^{-1},\gamma a\gamma^{-1}a^{-1},\gamma b\gamma^{-1}b^{-1}\rangle\cong\Z\times\Z\times\Z_2,
\] por lo que se trata de la suma conexa de tres planos proyectivos.
\end{sol*}
...
\end{exercise}
\begin{sol*}
En el primer caso, además de las relaciones de abelianización, obtendríamos $a_n^2$, ya que el resto se cancelaría con su inverso, por lo que simplemente nos quedaría un grupo isomorfo a $\Z_2$. Esto quiere decir que la primera superficie es un plano proyectivo. En el segundo caso, tras abelianizar todos se cancelan, por lo que el grupo fundamental es trivial y esto significa que la superficie es una esfera.
\end{sol*}
