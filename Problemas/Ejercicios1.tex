...
\end{itemize} 
% Entonces $\pi_B^{-1}(W)$ es cerrado por la definición de topología cociente, siendo $\pi_B$ la proyección canónica de $B$ en $B/A$. Observemos el %siguiente diagrama conmutativo:
%\[
%\begin{tikzcd}
...
Para la otra inclusión, supongamos que existe $[y]\in C_{[x]}$ tal que $C_y\neq C_x$. Tenemos que la imagen de cada componente conexa está contenida en una componente conexa, luego $\pi(C_y)\subset C_{[x]}$. Para la otra inclusión, supongamos que existe $[y]\in C_{[x]}$ tal 	que $y\in C_y\neq C_x$. Entonces existen $U$ y $V$ ambos clopen tales que $C_x\subset U$, $C_y\subset V$, $U\cup V=X$ y $U\cap V=\emptyset$. Nótese que $[x]$ solo se encuentra en uno de los dos, porque si no habríamos encontrado una partición de $[x]$. Como son clopen, son unión de componentes conexas, por lo que si $z\in U\Rightarrow [z]\subset U$ y lo mismo para $V$. Además se tiene que $\pi^{-1}\pi(U)=U$ (análogamente para $V$) por construcción, ya que $U = \bigcup \bigl\{ [z] : [z] \cap U \neq\emptyset\}$, por lo que $\pi(U)$ y $\pi(V)$ son clopen. Entonces $\pi(U)\cap C_{[x]}$ y $\pi(V)\cap C_{[x]}$ forman una partición por cerrados (ya que las componentes conexas son siempre cerradas y la intersección finita de cerrados es cerrada) de $C_{[x]}$, con lo cual hemos llegado a una contradicción.
\end{sol*}
