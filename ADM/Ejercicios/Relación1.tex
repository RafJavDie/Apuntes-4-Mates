...
$$
Aplicamos la fórmula del cambio de variable. Tomamos $v(\muestra{x})=$, entonces
$$f_\muestra{Y}(\muestra{x})=f_\muestra{X}(v(\muestra{x}))|v'(\muestra{x})| = \frac{1}{(2\pi)^{p/2}}\exp\{-1/2x'x\}
$$
...
$$
E[\muestra{X}]=E[\muestra{Y}\Sigma^{1/2}+\muestra{\mu}] = E[\muestra{Y}]\Sigma^{1/2}+\muestra{\mu}=\m{\mu}
$$
$$
Cov[\muestra{X}]=Cov[\muestra{Y}\Sigma^{1/2}+\muestra{\mu}] ={\Sigma^{1/2}}'Cov[\m{Y}]\Sigma^{1/2}= {\Sigma^{1/2}}\Sigma^{1/2} =\Sigma
$$
\item 
\begin{enumerate}[i.]
