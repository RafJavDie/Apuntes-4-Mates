...
\item Primer eje
\begin{align*}
\max_{u_1} & p'_{u_1}D_pp_{u_1} \\
s.a.& u_1'D_p^{-1}u_1 = 1
\end{align*}
\item Segundo eje
\begin{align*}
\max_{u_2} &\; p'_{u_2}D_pp_{u_2} \\
...
&=\sum_{\alpha=1}^p (u_\alpha D^{-1}_p r_i - u_\alpha D^{-1}_p r_{i'} )'(u_\alpha D^{-1}_p r_i - u_\alpha D^{-1}_p r_{i'} )\\
&=\sum_{\alpha=1}^p (r_i-r_{i'})'(u_\alpha D^{-1}_p )'(u_\alpha D^{-1}_p)(r_i-r_{i'})\\
...
La coordenada $i$-ésima viene dada por $m_r' D_p^{-1} u_i$. Claramente $m_r'D_p^{-1} = 1_p'$. Luego $m_r'D_p^{-1}u_i = u_i'1_p = 0$. ¿¿PREGUNTAR??
...
$$
Y anteriormente hemos deducido que $m_r'D_p^{-1}u_k =1_p'u_k = 0$. ¿INSISTO, PREGUNTAR?
\item Define la inercia total y demuestra que es igual a la suma de los autovalores de $S_r$.

Dfinimos la inercia total como $InT = \dfrac{\chi^2}{N}$. Esto es, por lo visto en teoría
$$
InT = \sum_{i=1}^n f_{i.}d_{\chi^2}(r_i,m_r)
$$
Además,
\begin{align*}
traza(S_r) &= \sum_{i=1}^n \lambda_i \\
